...
const UpdateHollowBlock = () => {
  ...
  const [hollowBlockData, setHollowBlockData] = useState(null);
  ...
  useEffect(() => {
    const fetchHollowBlock = async () => {
      try {
        ...
        setHollowBlockData(data);
        setValue('title', data.title);
        setValue('price', data.price);
        setValue('description', data.description);
      } catch (error) {
        console.error(
          'Error fetching hollow block:',
          error.response ? error.response.data : error.message
        );
        setMessage('Error fetching hollow block.');
      }
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response?.data) {
        setHollowBlockData(response.data.hollowblock);
        setMessage('Hollow block updated successfully! Redirecting...');
  ...
  return (
    <div className='bg-gray-100 min-h-screen '>
      ...
      <div className='pt-16 max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg'>
        <h2 className='text-2xl font-semibold mb-4'>Update Hollow Block</h2>
        ...
        {hollowBlockData?.image_url && (
          <div className='mt-6 text-center'>
            <h3 className='text-lg font-semibold mb-2'>Current Image:</h3>
            <img
              src={`${Api.BASE_URL}${hollowBlockData.image_url}`}
              alt='Current Hollow Block'
...
export default UpdateHollowBlock;
